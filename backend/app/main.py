...
async def analyze_multimodal_endpoint(request: Request):
    ...
    if "multipart/form-data" in content_type:
        ...
        # Accept a real base64-encoded file payload from the frontend.
        # Supports data-URI style ("data:video/mp4;base64,....") and raw base64 strings.
        b64_payload = (
            body.get("fileBase64")
            or body.get("fileData")
            or body.get("base64")
        )

        if b64_payload:
            if "," in b64_payload and b64_payload.strip().lower().startswith("data:"):
                b64_payload = b64_payload.split(",", 1)[1]
            try:
                import base64 as _b64
                file_bytes = _b64.b64decode(b64_payload)
            except Exception:
                raise HTTPException(
                    status_code=400,
                    detail="fileBase64 could not be decoded. Ensure it is valid base64 (with or without a data: URI prefix).",
                )
        else:
            # No real file content was supplied at all — this cannot be forensically analyzed.
            raise HTTPException(
                status_code=400,
                detail=(
                    "No file content received. Send the file as multipart/form-data, "
                    "or include a base64-encoded 'fileBase64' field in the JSON body."
                ),
            )
